 ...
 */
export async function writeChapter({ roleConfig, projectSummary, chapter, chapterIndex, reviewerIssues, reproduction, custom }) {
  const user = buildWriterUser({ projectSummary, chapter, chapterIndex, reviewerIssues, reproduction, custom });
 ...
 */
export async function writeAllChapters({ roleConfig, projectSummary, outline, outputDir, resume = false, reviewMap = {}, concurrency = 3, reproduction, custom }) {
  ...
  const workers = Array.from({ length: limit }, async () => {
    while (cursor < tasks.length) {
      ...
      const { content } = await writeChapter({
        roleConfig, projectSummary, chapter: t.chapter, chapterIndex: t.idx, reviewerIssues: t.reviewerIssues, reproduction, custom,
      });
